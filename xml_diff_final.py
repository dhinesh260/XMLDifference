...
def compare_elements(e1, e2, path=""):
    ...
    # Check for specific tags and count them
    if strip_namespace(e1.tag) == 'job' and strip_namespace(e2.tag) == 'job':
        skill_count1 = len([child for child in e1 if strip_namespace(child.tag) == 'skills'])
        skill_count2 = len([child for child in e2 if strip_namespace(child.tag) == 'skills'])
        
        skills1 = sum(1 for child in e1 if strip_namespace(child.tag) == 'skills' for _ in child)
        skills2 = sum(1 for child in e2 if strip_namespace(child.tag) == 'skills' for _ in child)

        if skills1 != skills2:
            differences["Tag Order Differences"].append(
                f"{path}/skills: {skills1} skill(s) found in file1.xml, but {skills2} skill(s) found in file2.xml."
            )
